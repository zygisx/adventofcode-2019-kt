...
fun buildGraph(map: Map): List<Vector> {
    ...
    val vectors = allKeys.map { start ->
        allKeys.filter { it != start }.mapNotNull { target ->
            findShortest(map, findCoordinates(start), target)
        }
    }.flatten()

    val starts = map.filter { it.value == '@' }
    assert(starts.size == 4)

    starts.keys.forEachIndexed { idx, p -> map[p] = (idx).toString().first()}
    return starts.keys.fold(vectors) {acc, point ->
        val allVectors = allKeys.mapNotNull { findShortest(map, point, it) }
        acc.plus(allVectors)
    }
}
...
fun collectAllKeys(graph: List<Vector>): Int {
    ...
    var currentRecord = Int.MAX_VALUE
    fun moveStartIdx(startIdx: Int) = if (startIdx == 3) 0 else startIdx + 1

    fun findShortestRec(starts: List<Char>, startIdx: Int, keys: Keyset, stepsSoFar: Int): Int {
        if (keys.size == totalKeys) {
            println("Terminating with ${keys.size} got $stepsSoFar $currentRecord")
            currentRecord = Math.min(stepsSoFar, currentRecord)
            return stepsSoFar
        ...
        val nextVectors = findNextVectors(graph, starts[startIdx], keys)
        if (nextVectors.isEmpty()) {
            return findShortestRec(starts, moveStartIdx(startIdx), keys,stepsSoFar)
        }

        val allDistances = nextVectors.map {
            val newStarts = starts.mapIndexed { idx, st -> if (idx == startIdx) it.to.key else st }
            findShortestRec(newStarts, startIdx, keys.plus(it.to.key), stepsSoFar + it.distance)
        }
        return allDistances.min()!!
    }

    return findShortestRec(listOf('0', '1', '2', '3'), 0, HashSet(), 0)
}
